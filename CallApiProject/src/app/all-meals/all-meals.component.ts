...
export class AllMealsComponent implements OnInit {
  ...
  constructor(private ser: MealService, private serU: UserService, public serc: MealCategoriesService) {
    this.GetAllMeals()
    ...
    this.ser.GetAllMeals().subscribe(succ => {
      ...
      this.listMeals.forEach(element => {
        ...
        this.ser.GetProductsMeal(element.mealCode).subscribe(succ => {
          ...
          if (this.serc.checkListCategories.length) {
            this.serc.checkListCategories.forEach(element => {
  ...
  sortByCategory(x: number) {
    this.listMeals.forEach(element => {
      if (element.mealCategoryCode == x && !this.meals.includes(element))
  ...
  check(x: MealCategories) {
    if (!this.serc.checkListCategories.includes(x)) { this.serc.checkListCategories.push(x) }
    else { this.serc.checkListCategories.splice(this.serc.checkListCategories.indexOf(x), 1) }
    this.meals = []
    if (this.serc.checkListCategories.length == 0)
      this.arr = this.listMeals
    this.serc.checkListCategories.forEach(element => {
      this.sortByCategory(element.mealCategoriesCode)
    });
    this.choose = this.serc.checkListCategories.length != 0 ? true : false;
  }
  search() {
    if (this.text == "") {
      this.meals = this.arr
